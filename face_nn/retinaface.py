...
from face_nn.nets.facenet import Facenet
from face_nn.nets_retinaface.retinaface import RetinaFace
from face_nn.utils.anchors import Anchors
from face_nn.utils.config import cfg_mnet, cfg_re50
from face_nn.utils.utils import (Alignment_1, compare_faces, letterbox_image,
                         preprocess_input)
from face_nn.utils.utils_bbox import (decode, decode_landm, non_max_suppression,
...
class Retinaface(object):
    _defaults = {
        ...
        #   retinaface训练完的权值路径
        #----------------------------------------------------------------------#
        "retinaface_model_path" : face_nn_path / 'model_data/Retinaface_mobilenet0.25.pth',
        #----------------------------------------------------------------------#
        ...
        #   facenet训练完的权值路径
        #----------------------------------------------------------------------#
        "facenet_model_path"    : face_nn_path / 'model_data/facenet_mobilenet.pth',
        #----------------------------------------------------------------------#
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        try:
            self.known_face_encodings = np.load(face_nn_path/f"model_data/{self.facenet_backbone}_face_encoding.npy")
            self.known_face_names     = np.load(face_nn_path/f"model_data/{self.facenet_backbone}_names.npy")
        except:
            if not encoding:
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        np.save(face_nn_path/f"model_data/{self.facenet_backbone}_face_encoding.npy",face_encodings)
        np.save(face_nn_path/f"model_data/{self.facenet_backbone}_names.npy",names)
    ...
    def recognize_faces(self, image):
        ...
        for face_encoding in face_encodings:
            ...
            if matches[best_match_index]:
                name = self.known_face_names[best_match_index]
            face_names.append({'name': name, 'distance': f"{face_distances[best_match_index]:.4f}"})
        return face_names
